...
export default function ProductAdmin({pImg, bgImg, title, desc, stock, price, oldPrice, category, slug, setTitle, setSlug, setDesc, setPrice, setOldPrice, setStock, setCategory, setEditOption, selectedFileRef}) {
    ...
    async function handleDeleteProduct() {
        await axios.post(`${import.meta.env.VITE_API_BASE_URL}/seller/product/delete`, {slug: slug}, { headers: { 'Content-Type': 'multipart/form-data', Authorization: `Bearer ${cookies.petx}` } }).then((res) => {
        ...
        <div className="flex border-2 py-2 rounded-lg">
            ...
            <div>
                ...
                <div className="flex space-x-2">
                    <div onClick={handleDeleteProduct} className="cursor-pointer w-20 bg-[#124C5F] text-white text-center font-inter font-semibold text-xs py-2 rounded-sm mt-1">Delete</div>
                    <div onClick={handleEditProduct} className="cursor-pointer w-20 bg-[#124C5F] text-white text-center font-inter font-semibold text-xs py-2 rounded-sm mt-1">Edit</div>
                </div>
            </div>
        </div>
    )
